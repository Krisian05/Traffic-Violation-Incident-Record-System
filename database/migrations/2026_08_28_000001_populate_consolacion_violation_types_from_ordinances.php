...
return new class extends Migration
{
    ...
    {
        $consolacion = Lgu::where('name', 'like', '%Consolacion%')->first();
        if (!$consolacion) {
            return;
        }
        ...
        DB::transaction(function () use ($consolacion, $consolacionLguId) {
            // Update ordinance reference on the Lgu record
            $consolacion->update([
                'ordinance_reference' => 'Ord. No. 34, S. 2025 as amended by Ord. No. 13, S. 2026 (Consolacion Revised Traffic Code)',
            ]);

            // Define the complete violations list from Res. No. 520 / Ord. No. 34, S. 2025 and Res. No. 255 / Ord. No. 13, S. 2026
            $violations = [
    ...
    public function down(): void
    {
        $consolacion = Lgu::where('name', 'like', '%Consolacion%')->first();
        if ($consolacion) {
            ViolationType::where('lgu_id', $consolacion->id)->delete();
            Cache::forget('violation_types');
            Cache::forget("violation_types_lgu_{$consolacion->id}");
        }
    }
};
