    ...
    <div class="mob-card-body">
        <form method="POST" action="{{ route('officer.incidents.update', $incident) }}" enctype="multipart/form-data" data-offline-sync="true" data-offline-record-type="incident-edit" data-offline-label="Incident Update">
            ...
            @include('partials.gps-locator', [
                'uid'      => 'gps_incident',
                'latValue' => $incident->gps_lat,
                'lngValue' => $incident->gps_lng,
            ])
            ...
            <div class="mb-3">
                <label class="mob-label">Status Change Note <span style="font-weight:400;color:#94a3b8;">(optional)</span></label>
                <input type="text" name="status_note" class="form-control mob-input @error('status_note') is-invalid @enderror"
...
function addOtherParty(data) {
    ...
    div.innerHTML = `
        ...
        <div class="mb-2">
            ...
            <input type="text" name="parties[${i}][name]" class="form-control mob-input"
                placeholder="Full name (optional)" value="${data ? (data.name || '') : ''}">
        </div>
        <div class="mb-2">
            <label class="mob-label">Contact Number</label>
            <input type="text" name="parties[${i}][contact_number]" class="form-control mob-input"
                placeholder="Contact number" value="${data ? (data.contact_number || '') : ''}">
        </div>
        <div class="mb-2">
            <label class="mob-label">Address</label>
            <input type="text" name="parties[${i}][address]" class="form-control mob-input"
                placeholder="Address (optional)" value="${data ? (data.address || '') : ''}">
        </div>
        <div>
            <label class="mob-label">Details / Statement</label>
            <input type="text" name="parties[${i}][description]" class="form-control mob-input"
                placeholder="Party type if 'Other', witness statement, injuries, remarks..." value="${data ? (data.description || '') : ''}">
        </div>`;
