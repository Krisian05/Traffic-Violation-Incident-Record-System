...
<head>
    ...
    <link rel="icon" type="image/png" sizes="192x192" href="{{ asset('images/app-icon-192.png') }}?v=4">
    <link rel="apple-touch-icon" href="{{ asset('images/app-icon.png') }}?v=4">
    ...
    <div id="mob-lb-stage" class="mob-lightbox-stage" onclick="event.stopPropagation()" style="position:relative;">
        ...
        <div style="display:flex;justify-content:flex-end;width:100%;margin-bottom:.4rem;">
            <button onclick="event.stopPropagation();mobLbClose()"
                style="width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.18);border:none;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;-webkit-tap-highlight-color:transparent;backdrop-filter:blur(8px);">
                <i class="ph ph-x" style="font-size:1rem;"></i>
            </button>
        </div>
        <img id="mob-lb-img" src="" alt="Photo" draggable="false" style="max-width:100%;max-height:72vh;border-radius:12px;box-shadow:0 8px 48px rgba(0,0,0,.6);display:block;touch-action:none;transform-origin:center center;">
        ...
        <div class="modal-content border-0 rounded-4 shadow" style="overflow:hidden;">
            <div style="background:linear-gradient(135deg,#1e3a8a,#1d4ed8);padding:.85rem 1.25rem;display:flex;align-items:center;justify-content:space-between;">
                <div style="display:flex;align-items:center;gap:.6rem;color:#fff;font-weight:800;font-size:.9rem;">
                    ...
                    <span>Fast QR / Barcode Scanner</span>
                </div>
            ...
            <div style="padding:1rem;background:#0f172a;text-align:center;">
                <div id="tvirs_capture_view">
                    ...
                    <div id="tvirs_scanner_status" style="margin-top:.7rem;font-size:.78rem;color:#94a3b8;font-weight:600;">Point camera at a barcode or QR code</div>
...
function tvirsProcessScannedCode(code, preParsed) {
    ...
    document.dispatchEvent(new CustomEvent('tvirs:license-scanned', {
        detail: { code: code || null, parsed: parsed, targetId: targetInputId, source: 'barcode' }
    }));
    ...
    navigator.mediaDevices.getUserMedia(constraints)
        .then(function(stream) {
            ...
            if ('BarcodeDetector' in window) {
                ...
                if (status) status.textContent = 'Barcode detector not supported on this browser.';
            }
        })
        .catch(function(err) {
...
function tvirsStopScanner() {
    if (tvirsScannerAnim) {
        cancelAnimationFrame(tvirsScannerAnim);
        tvirsScannerAnim = null;
    }
    if (tvirsScannerStream) {
