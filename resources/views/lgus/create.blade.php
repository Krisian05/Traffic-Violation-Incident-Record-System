    ...
    <form method="POST" action="{{ route('lgus.store') }}" enctype="multipart/form-data">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 p-3 bg-white rounded-3 shadow-sm border">
            <div class="d-flex align-items-center gap-3">
                <span class="lgu-header-icon">
                    <i class="bi bi-building-add" style="color:#fff;font-size:1.25rem;"></i>
                </span>
                <div>
                    <h5 class="fw-bold mb-0 text-dark">Add New LGU</h5>
                    <span class="text-muted small">Onboard a new municipality/city into the TVIRS network</span>
                </div>
            </div>
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary rounded-pill px-4 btn-sm fw-bold">
                    <i class="bi bi-check-lg me-1"></i> Save LGU
                </button>
            </div>
        </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="police_station_address" class="form-control lgu-input"
                                   ...
                                   value="{{ old('police_station_address') }}">
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <select name="sms_provider" class="form-select lgu-input" onchange="document.getElementById('create_tb_fields').style.display = (this.value === 'textbee' ? '' : 'none'); document.getElementById('create_sem_fields').style.display = (this.value === 'semaphore' ? '' : 'none');">
                                <option value="textbee" {{ old('sms_provider', 'textbee') === 'textbee' ? 'selected' : '' }}>📱 Android SIM Gateway (Textbee.dev — Free ₱0)</option>
                                <option value="semaphore" {{ old('sms_provider') === 'semaphore' ? 'selected' : '' }}>☁️ Semaphore SMS API (Paid Credits)</option>
                                <option value="local" {{ old('sms_provider') === 'local' ? 'selected' : '' }}>💻 Local Test Log Gateway (No SMS Sent)</option>
                            </select>
                        </div>

                        {{-- Textbee --}}
                        <div id="create_tb_fields" style="{{ old('sms_provider', 'textbee') === 'textbee' ? '' : 'display:none;' }}">
                            <div class="mb-3">
                                <label class="lgu-label">Textbee API Key</label>
                                <input type="password" name="textbee_api_key" class="form-control lgu-input" placeholder="e.g. tb_key_..." value="{{ old('textbee_api_key') }}">
                            </div>
                            <div class="mb-3">
                                <label class="lgu-label">Textbee Device ID</label>
                                <input type="text" name="textbee_device_id" class="form-control lgu-input" placeholder="e.g. 65a1b2c3..." value="{{ old('textbee_device_id') }}">
                            </div>
                        </div>

                        {{-- Semaphore --}}
                        <div id="create_sem_fields" style="{{ old('sms_provider') === 'semaphore' ? '' : 'display:none;' }}">
                            <div class="mb-3">
                                <label class="lgu-label">Semaphore SMS API Key</label>
                                <input type="password" name="sms_api_key" class="form-control lgu-input" placeholder="e.g. 9a8b7c6d..." value="{{ old('sms_api_key') }}">
                            </div>
                            <div class="mb-3">
                                <label class="lgu-label">SMS Sender Name</label>
                                <input type="text" name="sms_sender_name" class="form-control lgu-input" maxlength="11" placeholder="e.g. TVIRS" value="{{ old('sms_sender_name', 'TVIRS') }}">
                            </div>
                        </div>

                        <div class="form-check form-switch mt-3 pt-2 border-top">
                            <input class="form-check-input" type="checkbox" name="sms_auto_send" id="sms_auto_send" value="1" {{ old('sms_auto_send', true) ? 'checked' : '' }}>
        ...
        <div class="d-flex align-items-center justify-content-end gap-3 mt-4 pt-3 border-top">
            ...
            <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm">
                <i class="bi bi-check-lg me-1"></i> Save LGU Configuration
            </button>
        </div>
    </form>
</div>
