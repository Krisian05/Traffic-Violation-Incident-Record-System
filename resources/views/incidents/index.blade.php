...
.inc-status-under_assessment            { display:inline-block;padding:.1rem .5rem;border-radius:20px;font-size:.66rem;font-weight:700;background:#fffbeb;color:#92400e; }
.inc-status-assigned_for_investigation  { display:inline-block;padding:.1rem .5rem;border-radius:20px;font-size:.66rem;font-weight:700;background:#eef2ff;color:#4338ca; }
.inc-status-resolved                    { display:inline-block;padding:.1rem .5rem;border-radius:20px;font-size:.66rem;font-weight:700;background:#f0fdf4;color:#15803d; }
.inc-status-closed                      { display:inline-block;padding:.1rem .5rem;border-radius:20px;font-size:.66rem;font-weight:700;background:#f3f4f6;color:#374151; }
.inc-status-referred_to_authority       { display:inline-block;padding:.1rem .5rem;border-radius:20px;font-size:.66rem;font-weight:700;background:#faf5ff;color:#6b21a8; }
...
@section('topbar-sub')
    ...
    @php
        ...
        if ($status) {
            $statusDisplay = \App\Models\Incident::STATUSES[(string) $status] ?? (string) $status;
            ...
            <tbody>
                ...
                <tr class="inc-row" data-href="{{ route('incidents.show', $incident) }}">
                    ...
                    <td style="padding-left:1.75rem;">
                        ...
                        @endif
                        @php
                            $statusClass  = array_key_exists($incident->status, \App\Models\Incident::STATUSES)
                                ? 'inc-status-' . $incident->status
                                : 'inc-status-default';
                        @endphp
                        <div style="margin-top:.25rem;display:flex;align-items:center;gap:.4rem;">
                            <span class="{{ $statusClass }}">{{ $incident->statusLabel() }}</span>
